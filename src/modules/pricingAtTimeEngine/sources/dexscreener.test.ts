...
describe('fetchDexscreenerPriceDetailed — deterministic best-pair selection (dominant-holding price audit)', () => {
  it('pair ordering cannot change the selected price — reversing the API response array yields the identical winner', async () => {
    const pairA = { chainId: 'base', pairAddress: '0xAAA', priceUsd: '1.00', liquidity: { usd: 50_000 }, dexId: 'uniswap' }
    const pairB = { chainId: 'base', pairAddress: '0xBBB', priceUsd: '1.05', liquidity: { usd: 20_000 }, dexId: 'aerodrome' }
  ...
  it('a tie in liquidity (including the real all-zero-liquidity case) resolves deterministically by pair address, not by API response order', async () => {
    const tiedA = { chainId: 'base', pairAddress: '0xZZZ', priceUsd: '2.00', liquidity: { usd: 5000 }, dexId: 'uniswap' }
    const tiedB = { chainId: 'base', pairAddress: '0xAAA', priceUsd: '2.10', liquidity: { usd: 5000 }, dexId: 'aerodrome' }
  ...
  it('a wrong-chain pair is rejected even if it has far higher liquidity than the real matching pair', async () => {
    const wrongChain = { chainId: 'ethereum', pairAddress: '0xETH', priceUsd: '999.00', liquidity: { usd: 10_000_000 }, dexId: 'uniswap' }
    const correctChain = { chainId: 'base', pairAddress: '0xBASE', priceUsd: '1.50', liquidity: { usd: 2000 }, dexId: 'aerodrome' }
  ...
  it('a low-liquidity outlier pair is rejected — never used to price a token even when it is the only pair returned', async () => {
    const dustPair = { chainId: 'base', pairAddress: '0xDUST', priceUsd: '0.0001', liquidity: { usd: 5 } }
  ...
  it('a low-liquidity outlier is rejected in favor of a real liquid pair for the same token', async () => {
    const dustPair = { chainId: 'base', pairAddress: '0xDUST', priceUsd: '50.00', liquidity: { usd: 3 } }
    const realPair = { chainId: 'base', pairAddress: '0xREAL', priceUsd: '1.10', liquidity: { usd: 10_000 } }
  ...
  it('surfaces real pair provenance for a winning pair — pairAddress, dexId, liquidityUsd, pairAgeMs, quoteTokenSymbol, winnerReason', async () => {
    ...
    mockPairsResponse([{
      ...
      liquidity: { usd: 25_000 }, pairCreatedAt, quoteToken: { symbol: 'WETH' },
    }])
    const result = await fetchDexscreenerPriceDetailed('0xtoken', 'base', NOW)
  ...
  it('alternate valid pairs are surfaced (compact, capped) alongside the winner', async () => {
    mockPairsResponse([
      { chainId: 'base', pairAddress: '0xWIN', priceUsd: '1.00', liquidity: { usd: 100_000 } },
      { chainId: 'base', pairAddress: '0xALT', priceUsd: '1.02', liquidity: { usd: 40_000 } },
    ])
    const result = await fetchDexscreenerPriceDetailed('0xtoken', 'base', NOW)
