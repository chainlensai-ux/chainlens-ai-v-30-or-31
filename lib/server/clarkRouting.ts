...
export function getClarkAddressRouteHint(prompt: string): "token" | "wallet" | "ambiguous" | "none" {
  ...
  if (/^\/(lp|token|deployer|holders|explain)\b/.test(t)) return "token";
...
export function isTokenFollowupPrompt(prompt: string): boolean {
  ...
  if (/^\s*holders?\??\s*$/i.test(t)) return true;
  if (/^\s*who\s+deployed\s+(?:it|this|that)\??\s*$/i.test(t)) return true;
  if (/^\s*what\s+does\s+(?:lp|liquidity)\s+mean\??\s*$/i.test(t)) return true;
  if (/^\s*explain\s+lp\??\s*$/i.test(t)) return true;
  return TOKEN_FOLLOWUP_RE.test(t);
}

export type TokenFollowupKind = "safety" | "dev_rug" | "lp_lock" | "risk" | "analyst" | "holders" | "deployer";
...
export function classifyTokenFollowupKind(prompt: string): TokenFollowupKind {
  const t = String(prompt ?? "").toLowerCase();
  if (/^\s*holders?\??\s*$/i.test(t) || /\b(check\s+holders?|what\s+about\s+holders?|holder\s+check|holder\s+distribution|who\s+holds|holder\s+concentration|top\s+holders?)\b/.test(t)) return "holders";
  if (/\b(who\s+deployed(?:\s+(?:it|this|that))?|check\s+deployer|deployer\s+of|who\s+(?:made|created|built)\s+(?:this|it|that))\b/.test(t)) return "deployer";
  if (/\b(can\s+(?:the\s+)?dev\s+(?:rug|dump)|who\s+controls\s+(?:the\s+)?supply|explain\s+dev(?:\s+control)?)\b/.test(t)) return "dev_rug";
  if (/\b(is\s+lp\s+locked|is\s+it\s+locked|explain\s+lp|what\s+about\s+lp|what\s+about\s+liquidity|where\s+is\s+liquidity|can\s+liquidity\s+be\s+pulled|run\s+lp\s+check|lp\s+check|check\s+lp|liquidity\s+safety|check\s+liquidity(?:\s+safety)?|run\s+liquidity\s+check|is\s+liquidity\s+strong|is\s+that\s+enough\s+liquidity|liquidity\s+strong|liquidity\s+risk|enough\s+liquidity|exit\s+liquidity|how\s+much\s+liquidity|what\s+does\s+(?:lp|liquidity)\s+mean)\b/.test(t)) return "lp_lock";
...
export type ClarkSlashCommandName = "lp" | "token" | "wallet" | "base" | "deployer" | "holders" | "explain";
...
export function parseClarkSlashCommand(prompt: string): ClarkSlashCommand | null {
  ...
  const address = rest ? extractAddressForRouting(rest) : null;
  const addresses = rest ? extractAllAddressesForRouting(rest) : [];
  const symbol = slashSymbolFromRest(rest, address);
...
export function slashCommandQuestionCategory(prompt: string): "token" | "wallet" | null {
  ...
  if (slash.command === "lp" || slash.command === "token" || slash.command === "deployer" || slash.command === "holders" || slash.command === "explain") return "token";
  return null;
}
...
const DEPLOYER_CHECK_RE = /^(?:\/deployer\b|who\s+deployed(?:\s+(?:this|it|that|the\s+token))?\??|check\s+(?:the\s+)?deployer|deployer\s+(?:of|for|check)|who\s+(?:made|created|built)\s+(?:this|it|that))\b/i;

export function isHoldersCheckPrompt(prompt: string): boolean {
  const t = String(prompt ?? "").trim();
  if (!t) return false;
  if (/^\/holders\b/i.test(t)) return true;
  if (/^\s*holders?\??\s*$/i.test(t)) return true;
  return HOLDERS_CHECK_RE.test(t);
}

export function isDeployerCheckPrompt(prompt: string): boolean {
  const t = String(prompt ?? "").trim();
  if (!t) return false;
  if (/^\/deployer\b/i.test(t)) return true;
  if (/\bwho\s+deployed\b/i.test(t) || /\bcheck\s+(?:the\s+)?deployer\b/i.test(t) || /\bdeployer\s+of\b/i.test(t)) return true;
  return DEPLOYER_CHECK_RE.test(t);
}
...
export function resolveSlashCommandMemoryTarget(input: {
  ...
  if (input.command === "lp" || input.command === "token" || input.command === "deployer" || input.command === "holders" || input.command === "explain") {
...
export function classifyClarkPrompt(prompt: string): {
  ...
  // Command-first: /lp /token /wallet /base /deployer /holders /explain bypass generic TOKEN READ / wallet-default routing.
  if (slash) {
    return {
      intent: slash.intent,
  ...
  if (isHoldersCheckPrompt(raw)) {
    return { intent: "holders_check", address, addresses, deep: false, symbol: address ? null : symbol };
  }
  if (isDeployerCheckPrompt(raw)) {
    return { intent: "deployer_check", address, addresses, deep: false, symbol: address ? null : symbol };
  }
...
/** Real token-answer CTAs. Deep Scan Token is not a feature — never included. */
export function buildClarkTokenAnswerActions(tokenAddress: string, chainLabel = "base"): ClarkUiAction[] {
...
/** Real LP-answer CTAs. Deep Scan Token is not a feature — never included. */
export function buildClarkLpAnswerActions(tokenAddress: string, chainLabel = "base"): ClarkUiAction[] {
  const href = tokenScannerHref(tokenAddress, chainLabel);
  return [
    { label: "/holders", prompt: "/holders", kind: "prompt" },
    { label: "/deployer", prompt: "/deployer", kind: "prompt" },
...
/** Real wallet-answer CTAs. Deep Scan Wallet is the only valid Deep Scan action. */
export function buildClarkWalletAnswerActions(walletAddress: string): ClarkUiAction[] {
  const href = `/terminal/wallet-scanner?address=${encodeURIComponent(walletAddress)}&chain=auto`;
  return [
    { label: "Deep Scan Wallet", prompt: "deep scan it", kind: "prompt" },
    { label: "Open Wallet Scanner", href, kind: "link" },
    { label: "Track Wallet", prompt: "track this wallet", kind: "prompt" },
    { label: "Explain PnL", prompt: "explain pnl", kind: "prompt" },
  ];
}
...
export type TokenScanEvidence = {
  ...
  warnings?: string[];
  ok?: boolean;
  devIntel?: Record<string, unknown> | null;
  deployerProfile?: Record<string, unknown> | null;
  ownership?: Record<string, unknown> | null;
  supplyControl?: Record<string, unknown> | null;
  clusterMap?: Record<string, unknown> | null;
  previousProjects?: Array<Record<string, unknown>>;
...
const SOLANA_CLARK_TOKEN_NEXT_ACTIONS = ["/lp", "/holders", "/deployer", "/explain lp", "Add to Watchlist", "Open Token Scanner"];
...
export function formatHoldersCheck(ev: TokenScanEvidence, chain = "Base"): string {
  ...
  const addr = ev.token?.address ?? null;
  const h = ev.holders;
  const top1 = h?.top1 ?? null;
  const top10 = h?.top10 ?? null;
  const count = h?.holderCount ?? null;
  const missing: string[] = [];
  if (top1 == null) missing.push("top holder percent");
  if (top10 == null) missing.push("top 10 percent");
  if (count == null) missing.push("holder count");
  const conc =
    top1 != null && top1 >= 40 ? "High — a single wallet dominates supply."
  ...
  return [
    ...
    ...(addr ? [`Address: ${addr}`] : []),
    "",
    `Top holder: ${top1 != null ? `${top1.toFixed(1)}%` : "unverified"}`,
    `Top 10: ${top10 != null ? `${top10.toFixed(1)}%` : "unverified"}`,
    `Holder count: ${count != null ? count.toLocaleString() : "unverified"}`,
    ...
    "",
    "Missing evidence:",
    ...(missing.length > 0 ? missing.map((m) => `- ${m}`) : ["- none — top holder, top 10, and holder count returned"]),
    "",
    "Next:",
    "- /lp",
    "- /deployer",
...
function liquiditySection(ev: TokenScanEvidence): RiskSection {
  ...
  if (isConcentratedLp(lp)) {
    ...
    return {
      ...
      why: "Concentrated (v3/v4-style) pools don't mint lock/burn-provable LP tokens, so control must be confirmed a different way. Confirmed pool depth does not by itself confirm who controls that liquidity.",
      openCheck: !proof.hasProof && !depthVerified,
    };
  }
  ...
  return { title: "Liquidity / LP", status: `${depth} LP/control proof: ${lp.status}${lp.reason ? ` — ${lp.reason}` : ""}.`, why: "LP control determines whether liquidity can be removed unilaterally." };
}
...
function securityMissingReasonText(reason: string | null | undefined): string {
  if (reason === "auth_failed") return "security simulation could not be authenticated";
  if (reason === "security_simulation_timed_out") return "security simulation timed out";
  if (reason === "security_simulation_failed") return "security simulation failed";
  if (reason === "unsupported_chain") return "security simulation is not supported on this chain";
  return "security simulation unavailable";
}

function securityHoneypotSection(ev: TokenScanEvidence): RiskSection {
  const sec = ev.security;
  if (sec?.honeypot == null) {
    const reasonText = securityMissingReasonText(sec?.missingReason);
 ...
 */
/** Evidence-coverage map for the clarkEvidenceCoverage debug field — "verified" once the
 * underlying field is confirmed one way or the other, "open_check" only when genuinely unknown. */
export function tokenEvidenceCoverage(ev: TokenScanEvidence | null | undefined): Record<string, "verified" | "open_check"> {
  const mkt = ev?.market, lp = ev?.lpControl, sec = ev?.security, h = ev?.holders;
  return {
    lpDepth: mkt?.liquidity != null ? "verified" : "open_check",
    lpControl: lp?.status && lp.status !== "open_check" && lp.status !== "unverified" ? "verified" : "open_check",
    security: sec?.honeypot != null ? "verified" : "open_check",
    honeypot: sec?.honeypot != null ? "verified" : "open_check",
    deployerIdentity: ev?.deployerAddress ? "verified" : "open_check",
    linkedWallets: (ev?.linkedWallets?.length ?? 0) > 0 ? "verified" : "open_check",
    previousTokens: (ev?.previousProjects?.length ?? 0) > 0 ? "verified" : "open_check",
    fundingPatterns: "open_check",
    holderDistribution: h?.top1 != null || h?.top10 != null ? "verified" : "open_check",
    ownership: sec?.ownerRenounced != null ? "verified" : "open_check",
  };
}

/** Computes the 6 risk-report sections without formatting — used to derive the
 * clarkOpenCheckCount/clarkOpenCheckReasons debug fields from the same source of truth. */
export function tokenRiskSections(ev: TokenScanEvidence): RiskSection[] {
  return [
    liquiditySection(ev),
    ownershipSection(ev),
...
export type DevHistoryStatus = "confirmed_rug" | "risk_signals" | "no_evidence" | "open_check";

const DEV_HISTORY_ADDR_RE = /^0x[a-fA-F0-9]{40}$/;
function devHistoryObj(value: unknown): Record<string, unknown> { return value && typeof value === "object" && !Array.isArray(value) ? value as Record<string, unknown> : {}; }
function devHistoryArray(value: unknown): unknown[] { return Array.isArray(value) ? value : []; }
function devHistoryAddr(value: unknown): string | null { return typeof value === "string" && DEV_HISTORY_ADDR_RE.test(value.trim()) ? value.trim() : null; }
function devHistoryNum(value: unknown): number | null { return typeof value === "number" && Number.isFinite(value) ? value : null; }
...
export type DevHistoryEvidence = {
  ...
  statusReason: string;
  inputType: "Token contract" | "Dev wallet" | "Unknown";
  chain?: string | null;
  address?: string | null;
  ...
  confidence: string;
  tokenLocalRiskSignals: string[];
  previousLaunchedTokens: string[];
  repeatedRiskyPatterns: string[];
  linkedWalletClusterSignals: string[];
  suspiciousFundingPatterns: string[];
  priorConfirmedRugEvidence: string[];
  evidenceGaps: string[];
  sourcesUsed: string[];
  apiPathsUsed: string[];
  ...
  /** True once dev-wallet evidence was actually fetched, even if it came back empty — lets
   * formatters say "none found" instead of "Open Check" for genuinely-checked-but-empty fields. */
...
export function formatDevHistoryRead(opts: {
  ...
  return [
    ...
    `- linked-wallet/cluster signals: ${checkedOrFallback(linkedWalletClusterSignals, "None found in available dev-wallet evidence")}`,
...
export function deriveDevHistoryFromTokenEvidence(ev: TokenScanEvidence | null | undefined, walletEvidence?: Record<string, unknown> | null): DevHistoryEvidence {
  ...
  const clusterRiskLabel = String(devHistoryObj(ev.clusterMap).summary ? devHistoryObj(devHistoryObj(ev.clusterMap).summary).clusterRiskLabel : "").toLowerCase() || String(devHistoryObj(devHistoryObj(wallet.clusterMap).summary).clusterRiskLabel ?? "").toLowerCase();
  const clusterIsRisky = clusterRiskLabel === "elevated" || clusterRiskLabel === "high" || clusterRiskLabel === "critical";
  ...
  // Linked-wallet existence alone is context, not risk — only confirmed risky linked-wallet
  // behavior, an elevated/high/critical cluster-risk label, or a meaningful cluster supply share
  // counts as a cross-token risk signal.
  const linkedWalletRiskConfirmed = linkedWalletRiskCount > 0 || clusterIsRisky || (clusterRiskScore != null && clusterRiskScore >= 70) || (clusterSupply != null && clusterSupply >= 10);
  ...
  const evidenceGaps = [!deployer ? "deployer not confirmed" : null, previousLaunchedTokens.length === 0 && !walletEvidence ? "previous token history unavailable" : null, linkedWallets.length === 0 && !walletEvidence ? "linked wallet evidence unavailable" : null, !walletEvidence ? "wallet activity unavailable" : null].filter((v): v is string => Boolean(v));
...
//
// MERGE NOTE, DISCLOSED: a separate, concurrent fix (origin/main) independently added
// "what did whales buy/sell" / "what are whales buying/selling" to WHALE_RECENT_RE below. Those
// phrasings are deliberately NOT in WHALE_RECENT_RE here — routing them to the plain "recent
// activity" answer instead of the directional buy/sell answer below would give a strictly worse
// response (raw feed instead of a real per-token/side breakdown from the same rows), so
// WHALE_RECENT_RE keeps only its own non-overlapping additions (latest/accumulating wording) and
// this directional pair owns the SINGLE-direction buy/sell phrasing.
//
// BOTH-SIDES GUARD, DISCLOSED (found while merging with a concurrent fix that added "what did
// whales buy/sell" to WHALE_RECENT_RE — that phrasing genuinely asks about BOTH sides at once, so
// the plain recent-activity answer is the right one, not a single-direction breakdown of just one
// side): WHALE_BOTH_SIDES_RE catches "buy/sell"-shaped combined phrasing so it's excluded from the
// single-direction matchers below and instead falls through to WHALE_RECENT_RE's own
...
export function classifyClarkToolIntent(prompt: string): ClarkToolIntentResult {
  ...
  // WHALE_BOTH_SIDES_RE's own header for why "buy/sell"-shaped phrasing must never be captured as
  // one-sided. Falls through to WHALE_SUMMARY_RE below (the general feed answer), never a
  // single-direction one.
  if (!WHALE_BOTH_SIDES_RE.test(t)) {
...
const TRACK_WALLET_RE = /\b(track\s+(?:this\s+|that\s+)?wallet|add\s+(?:this\s+|that\s+)?wallet\s+to\s+(?:my\s+)?watchlist)\b/i;

export function isClarkTrackWalletCommand(prompt: string): boolean {
  const t = String(prompt ?? "").trim();
  return TRACK_WALLET_RE.test(t);
}
