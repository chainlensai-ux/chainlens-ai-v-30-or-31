...
const WHALE_RE = /\b(?:latest\s+whale\s+alerts?|whale\s+alerts?|show\s+(?:me\s+)?(?:the\s+)?(?:base\s+)?whales?|base\s+whales?|what\s+did\s+whales?\s+(?:buy|sell)|what\s+are\s+whales?\s+(?:buying|selling)|wallets?\s+(?:are\s+)?accumulating|accumulating\s+wallets?|explain\s+(?:this\s+|that\s+)?whale\s+alert|which\s+(?:whale\s+)?alerts?\s+matter\s+most)\b/i;
