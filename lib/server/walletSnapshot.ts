...
export type WalletSnapshot = {
  ...
  reconstructedClosedLots?: number
  ...
  _diagnostics?: {
    ...
    walletSwapDetectionDebug?: {
      ...
      // WALLET-ATTR-FIX-1 before/after diagnostics: what recoveredUnknownDirectionEvents and
      // recoveredSwapContextTransactions would have been under the pre-fix strict
      // walletIsInitiator-only gate. Comparing *BeforeFix to the unsuffixed fields shows
...
export type WalletClosedLot = {
  ...
  coveragePercent?: number
  missingReasons?: string[]
  // PRICE-INDEPENDENCE-FIX: see computePriceIndependence — a closed lot can only be treated as
  // a decisive/verified PnL data point when its entry and exit prices come from independent real
  // evidence; otherwise it is a fake flat (entry===exit) break-even, not a real zero-PnL trade.
  entryPriceSource?: string
  exitPriceSource?: string
...
function buildSwapDetection(evidenceList: WalletTxEvidence[], activityRequested: boolean, walletAddress: string): {
  ...
  type TxCtx = {
    ...
    hasStableLeg: boolean
    hasWethLeg: boolean
    walletParticipatedInTx: boolean
    swapContextCandidateStrict: boolean
  ...
  for (const [txHash, group] of byTx.entries()) {
    ...
    // WALLET-ATTR-FIX-1: "wallet initiated" was gated solely on tx.from === wallet
    // (walletIsInitiator), but a resolved hasBuy/hasSell leg already proves the wallet's own
    // Transfer fromAddress/toAddress matched the wallet directly (see direction assignment in
    // fetchGoldrushPnlEvents) — strictly stronger, narrower evidence than the tx-level sender
    // heuristic. Relayed/aggregator/meta-tx/smart-account swaps have tx.from !== wallet even
    // though the wallet is the genuine swap counterparty, which previously zeroed out
    // walletParticipatedInTx and therefore swapContextCandidate for the very transactions whose
    // other (pool-internal) legs are unknown-direction. swapContextCandidateStrict is retained
    // unchanged for before/after diagnostics; it does not affect any classification, FIFO,
    // pricing, scoring, or public output — those still depend only on the existing
    // swapDetection/isSwapCandidate logic untouched below.
    const walletParticipatedInTx = walletIsInitiator || hasBuy || hasSell
    const swapContextCandidateStrict = walletIsInitiator && (hasStableLeg || hasWethLeg || hasBuy || txToKnownRouter)
    // swapContextCandidate is diagnostic-only context, never used to promote an event to a
    // swap candidate or to alter direction/FIFO/pricing/PnL: the wallet genuinely participated
    // in the tx (as sender, or via a proven wallet-side leg) AND (a stablecoin or WETH quote
    // leg exists in the tx, OR the wallet received a token in the tx, OR the tx called a known
    // router).
    const swapContextCandidate = walletParticipatedInTx && (hasStableLeg || hasWethLeg || hasBuy || txToKnownRouter)
    txCtxMap.set(txHash, { txToAddr, txFromAddr, txRouterProtocol, txToKnownRouter, walletIsInitiator, hasBuy, hasSell, hasInboundOutbound, txHasStableOrWeth, distinctContracts, hasMultipleDistinctTokens, group, hasStableLeg, hasWethLeg, walletParticipatedInTx, swapContextCandidateStrict, swapContextCandidate })
  }
  ...
  let recoveredSwapContextTransactionsBeforeFixCount = 0
  let recoveredRouterTransactionsCount = 0
  let recoveredStableLegTransactionsCount = 0
  let recoveredWethLegTransactionsCount = 0
  for (const ctx of txCtxMap.values()) {
    if (ctx.swapContextCandidateStrict) recoveredSwapContextTransactionsBeforeFixCount++
  ...
  // WALLET-ATTR-FIX-1 before/after diagnostic: what the count above would have been under the
  // pre-fix strict walletIsInitiator-only gate (swapContextCandidateStrict). Comparing this to
  ...
  const evidenceWithDetection: WalletTxEvidence[] = evidenceList.map(e => {
    ...
    if (e.direction === 'unknown') {
      ...
      if (ctx?.swapContextCandidateStrict) recoveredUnknownDirectionEventsBeforeFixCount++
      if (ctx?.swapContextCandidate) {
        ...
        // WALLET-ATTR-FIX-1: surface when recovery relied on the relaxed wallet-participation
        // gate (proven wallet-side leg in this tx) rather than tx.from === wallet, so the
        // distinction is visible in diagnostics without changing scoring/confidence rules.
        const recoveredViaParticipationOnly = !ctx.walletIsInitiator && (ctx.hasBuy || ctx.hasSell)
        const initiatorClause = recoveredViaParticipationOnly
          ? 'wallet-participated transaction (tx sender differs from wallet — likely relayed/aggregator-initiated, but wallet-side leg proven)'
          : 'wallet-initiated transaction'
        walletAttributionRecoveryReason = `${initiatorClause} with ${recoveryReasons.join('; ')}`
...
// PRICE-INDEPENDENCE-FIX: sources with no real per-event price evidence — they reuse a
...
function classifyClosedLotForPublicPerformance(lot: WalletClosedLot, walletValueUsd: number): {
  reconstructedEligible: boolean
  verifiedPnlEligible: boolean
  performanceEligible: boolean
  rejectReason?: string
  priceEvidenceClass: 'independent_quote_legs' | 'mixed_independent' | 'same_price_flat_estimate' | 'synthetic_missing_cost_basis' | 'missing_independent_price' | 'stable_or_treasury_spread' | 'dust_or_micro_lot' | 'unknown'
} {
  const missingReasons = lot.missingReasons ?? []
  const synthetic = lot.evidence?.entrySource === 'synthetic' || missingReasons.includes('fifo_backfilled_buy') || missingReasons.includes('price_synthetic_fifo') || (lot.coveragePercent ?? 100) === 0
  if (synthetic || lot.costBasisUsd == null || lot.costBasisUsd <= 0) return { reconstructedEligible: true, verifiedPnlEligible: false, performanceEligible: false, rejectReason: 'synthetic_cost_basis_missing', priceEvidenceClass: 'synthetic_missing_cost_basis' }
  const symbol = (lot.tokenSymbol ?? '').toUpperCase()
  const stable = /^(USDC|USDT|DAI|USDE|USDS|FRAX|LUSD|PYUSD|GUSD|TUSD|USD\+|USDBC|USDB)$/.test(symbol)
  const dustThreshold = Math.max(5, Math.min(25, walletValueUsd * 0.00002))
  if (lot.costBasisUsd < dustThreshold) return { reconstructedEligible: true, verifiedPnlEligible: true, performanceEligible: false, rejectReason: 'dust_or_micro_lot', priceEvidenceClass: 'dust_or_micro_lot' }
  if (lot.pnlDisplayStatus === 'estimate_only_price_flat') return { reconstructedEligible: true, verifiedPnlEligible: false, performanceEligible: false, rejectReason: 'estimate_only_price_flat', priceEvidenceClass: 'same_price_flat_estimate' }
  if (lot.priceIndependenceStatus === 'missing_independent_price' || lot.priceIndependenceStatus === 'current_price_reused' || lot.priceIndependenceStatus === 'fallback_price_reused') return { reconstructedEligible: true, verifiedPnlEligible: false, performanceEligible: false, rejectReason: 'missing_independent_price', priceEvidenceClass: 'missing_independent_price' }
  const maxAbsPrice = Math.max(1, Math.abs(lot.entryPriceUsd ?? 0), Math.abs(lot.exitPriceUsd ?? 0))
  const nearEqualPrice = Math.abs((lot.entryPriceUsd ?? 0) - (lot.exitPriceUsd ?? 0)) <= maxAbsPrice * 0.0005
  const nearZeroPnl = Math.abs(lot.realizedPnlUsd ?? 0) < Math.max(0.05, lot.costBasisUsd * 0.0005)
  if (nearEqualPrice || nearZeroPnl) return { reconstructedEligible: true, verifiedPnlEligible: lot.pnlDisplayStatus === 'verified_pnl', performanceEligible: false, rejectReason: 'same_price_or_near_zero_pnl', priceEvidenceClass: 'same_price_flat_estimate' }
  if (stable && Math.abs(lot.realizedPnlUsd ?? 0) < Math.max(2, lot.costBasisUsd * 0.005)) return { reconstructedEligible: true, verifiedPnlEligible: true, performanceEligible: false, rejectReason: 'stable_or_treasury_spread', priceEvidenceClass: 'stable_or_treasury_spread' }
  const fastShuffle = (lot.holdingTimeSeconds ?? 999999) < 300 && Math.abs(lot.realizedPnlUsd ?? 0) < Math.max(5, lot.costBasisUsd * 0.001)
  if (fastShuffle) return { reconstructedEligible: true, verifiedPnlEligible: true, performanceEligible: false, rejectReason: 'routing_or_treasury_shuffle', priceEvidenceClass: 'stable_or_treasury_spread' }
  const priceEvidenceClass = lot.priceIndependenceStatus === 'independent_quote_legs' ? 'independent_quote_legs' : lot.priceIndependenceStatus === 'mixed_independent' || lot.priceIndependenceStatus === 'independent_provider_prices' ? 'mixed_independent' : 'unknown'
  return { reconstructedEligible: true, verifiedPnlEligible: lot.pnlDisplayStatus === 'verified_pnl', performanceEligible: lot.pnlDisplayStatus === 'verified_pnl', priceEvidenceClass }
}
...
function integrityCheckPnl(input: {
...
}): { ok: boolean; errors: string[]; warnings: string[]; violations: string[]; status: 'ok' | 'warning' | 'invalid' } {
  ...
  const status: 'ok' | 'warning' | 'invalid' = isInvalid ? 'invalid' : isSuspicious ? 'warning' : 'ok'
  ...
  const errors = status === 'invalid' ? violations : []
  const warnings = status === 'warning' ? violations : []
  return { ok: status === 'ok', errors, warnings, violations: errors, status }
}
  ...
  for (const e of eligible) {
    ...
    if (e.direction === 'buy') {
      ...
      while (sellRemaining > lotEpsilonFor(amount) && queue.length > 0) {
        ...
        const _priceIndependence = computePriceIndependence(lot.priceSource, priceSource, lot.entryPriceUsd, priceUsd, lot.openedTxHash, e.txHash!)
        closedLots.push({
          tokenAddress: e.contract.toLowerCase(),
          tokenSymbol: e.symbol ?? null,
          chain: normalChain,
...
function buildTradeStatsSummary(
  ...
  const winRateComputed = n >= WIN_RATE_THRESHOLD && decisiveClosedLots >= 1 && !isBreakEvenOnly && economicallyMeaningful && !_allLotsFlatEstimate
  ...
  // PRICE-INDEPENDENCE-FIX: when every closed lot is a same-source flat-price estimate, there is
  // no independently-verified price movement at all — report open_check, not a fake "ok" PnL.
  if (_allLotsFlatEstimate) {
    summaryStatus = 'open_check'
    confidenceLabel = 'open_check'
    sampleWarning = 'Closed lots found, but price movement is not independently verified.'
  }
  ...
  if (_flatEstimateLots.length > 0) missing.push('price_flat_estimate_lots_excluded')
  if (_allLotsFlatEstimate) missing.push('all_closed_lots_price_flat_estimate')
  ...
  const enrichedEvidence: WalletTxEvidence[] = evidenceWithDetection.map(e => {
    ...
    return {
      ...
      swapDetection: {
        ...
        matchedProtocol: null, matchedAddress: d.txTo, swapReconstructionConfidence: 'medium' as const,
      } satisfies WalletSwapDetection,
    }
  })
...
export async function fetchWalletSnapshot(address: string, options: WalletSnapshotOptions = {}): Promise<WalletSnapshot> {
  ...
  // WALLET-NO-PNL-UX-2: a normal_wallet with wallet-initiated activity but zero swap candidates
  // (and no synthetic lots either) gets the same cheap receipt-level reconstruction, sourced from
  // the existing value-ranked target tokens instead of synthetic-lot targets. Same function, same
  // 2-receipt cap, same reused candidate tx hashes — no new provider calls, no raised caps.
  // walletFacts.sourceClassification (walletAddressType) isn't built yet at this point in the
  // function, so non-trader addresses are excluded downstream instead — see
  ...
  const walletPnlRecoveryV2Debug: WalletPnlRecoveryV2Debug = _pnlRecoveryV2Result
    ...
    : {
        attempted: false,
        reason: !_baseReconChainOk ? 'eth_chain'
          : !baseRpcUrl ? 'no_rpc_url'
          : _realBackedClosedLotsCountForV2 > 0 ? 'already_has_real_backed_closed_lots'
          : (_syntheticClosedLots.length === 0 && !_noSwapCandidateRecoveryEligible) ? 'no_synthetic_lots_and_no_swap_candidate_targets'
          : 'not_eligible',
  ...
  const _publicLotClassifications = _syntheticLotsAfterSourceLots.map(l => ({ lot: l, classification: classifyClosedLotForPublicPerformance(l, totalValue) }))
  const _verifiedPnlClosedLotsFinal = _publicLotClassifications.filter(x => x.classification.verifiedPnlEligible).map(x => x.lot)
  const _estimateOnlyClosedLotsFinal = _realBackedClosedLotsFinal.filter(l => l.pnlDisplayStatus === 'estimate_only_price_flat')
  const _performanceClosedLotsFinal = _publicLotClassifications.filter(x => x.classification.performanceEligible).map(x => x.lot)
  const _decisivePnlClosedLotsFinal = _performanceClosedLotsFinal
  const _verifiedIndependentClosedLotsFinal = _verifiedPnlClosedLotsFinal.length
  const _flatPriceClosedLotsExcludedFinal = _publicLotClassifications.filter(x => x.classification.rejectReason === 'same_price_or_near_zero_pnl' || x.classification.rejectReason === 'estimate_only_price_flat').length
  const _excludedBreakEvenLotsFinal = _publicLotClassifications.filter(x => !x.classification.performanceEligible && Math.abs(x.lot.realizedPnlUsd ?? 0) < Math.max(0.05, (x.lot.costBasisUsd ?? 0) * 0.0005)).length
  const _performanceStatsResult = buildTradeStatsSummary(_performanceClosedLotsFinal, activityRequested, totalValue)
  for (const k of _performanceStatsResult.quarantinedLotKeys) _quarantinedLotKeys.push(k)
  const _performanceStats = _performanceStatsResult.summary
  const _performanceRealizedPnlUsd = _performanceClosedLotsFinal.length > 0 ? _performanceClosedLotsFinal.reduce((sum, l) => sum + l.realizedPnlUsd, 0) : null
  const _performanceCostBasisUsd = _performanceClosedLotsFinal.reduce((sum, l) => sum + (l.costBasisUsd ?? 0), 0)
  const _nearFlatPublicSample = _performanceClosedLotsFinal.length > 0 && Math.abs(_performanceRealizedPnlUsd ?? 0) < Math.max(25, _performanceCostBasisUsd * 0.0025)
  const _limitedVerifiedPublicSample = _performanceClosedLotsFinal.length > 0 && _performanceClosedLotsFinal.length < 10
  const _publicPnlStatusFinal: NonNullable<WalletSnapshot['publicPnlStatus']> = _performanceClosedLotsFinal.length === 0
    ? (_estimateOnlyClosedLotsFinal.length > 0 ? 'flat_estimate_only' : 'open_check')
    : _nearFlatPublicSample ? 'near_flat_verified_sample'
      : _limitedVerifiedPublicSample ? 'limited_verified_sample' : 'ok'
  const _publicPnlStatusReasonFinal = _publicPnlStatusFinal === 'near_flat_verified_sample'
    ? `Verified FIFO sample is near-flat (${_performanceClosedLotsFinal.length} performance-grade lots; ${_syntheticLotsExcludedFromStatsFinal + _estimateOnlyClosedLotsFinal.length + _flatPriceClosedLotsExcludedFinal} excluded), so it is not enough profitable evidence to call skill.`
    : _publicPnlStatusFinal === 'limited_verified_sample' ? `${_performanceClosedLotsFinal.length} performance-grade FIFO lots found, but sample size is below the win-rate/skill threshold and coverage is partial.`
  ...
  // PUBLIC-PNL-TRUTH: public trade stats are based only on performance-grade lots. Raw and
  // reconstructed counts remain visible separately for audit, but win rate/profile/window inputs
  // never use synthetic, flat, dust, or missing-independent-price lots.
  if (_publicPnlStatusFinal !== 'open_check' && _performanceClosedLotsFinal.length > 0) {
    promotedTradeStatsSummary = {
      ..._performanceStats,
      closedLotsForStats: _performanceClosedLotsFinal.length,
      verifiedClosedLots: _verifiedIndependentClosedLotsFinal,
      ...
      syntheticClosedLotsExcluded: _syntheticLotsExcludedFromStatsFinal,
      pnlUnavailableReason: _publicPnlStatusFinal === 'near_flat_verified_sample' ? 'near_flat_verified_sample' : _publicPnlStatusFinal === 'limited_verified_sample' ? 'limited_verified_sample' : null,
  ...
  const _toClosedTradeSample = (l: WalletClosedLot): WalletSnapshot['walletClosedTradeSamples'][number] => {
    ...
    // PRICE-INDEPENDENCE-FIX: a same-source flat-price estimate is never "verifiable", even when
    // it's real-backed and carries both tx hashes — its entry/exit prices aren't independent
  ...
  const _pnlQualityReason: string =
    ...
    : _pnlQuality === 'fifo_with_estimates' && _hasFlatEstimateExclusions ? 'verified_fifo_with_estimate_lots_excluded'
  ...
  const _walletAddressTypeForRecovery = walletFacts.sourceClassification?.walletAddressType ?? 'unknown'
  const _walletIsNonTraderAddressType = _walletAddressTypeForRecovery === 'token_contract_like' || _walletAddressTypeForRecovery === 'treasury_or_distributor_like'
  ...
  const snapshot: WalletSnapshot = {
    ...
    walletBehavior,
    estimatedPnl: _performanceClosedLotsFinal.length > 0 ? { ...estimatedPnl, publicUse: false, displayLabel: 'legacy_average_cost_estimate', reason: 'Legacy average-cost estimate is not used for public PnL when FIFO public performance evidence exists.' } : estimatedPnl,
    rawClosedLots: _syntheticLotsAfterSourceLots.length,
    reconstructedClosedLots: _syntheticLotsAfterSourceLots.length,
    syntheticClosedLotsExcluded: _syntheticLotsExcludedFromStatsFinal,
    estimateOnlyClosedLots: _estimateOnlyClosedLotsFinal.length,
    flatPriceClosedLotsExcluded: _flatPriceClosedLotsExcludedFinal,
    verifiedPnlClosedLots: _verifiedIndependentClosedLotsFinal,
    decisivePnlClosedLots: _decisivePnlClosedLotsFinal.length,
    performanceClosedLots: _performanceClosedLotsFinal.length,
    publicClosedLots: _performanceClosedLotsFinal.length,
    publicPerformanceClosedLots: _performanceClosedLotsFinal.length,
    ...
    walletClosedLotsAll: _performanceClosedLotsFinal,
    walletHistoricalCoverageSummary,
    walletHistoricalCandidateSummary,
    walletHistoricalPricingPreviewSummary,
    walletHistoricalFifoPreviewSummary,
  ...
  try {
    ...
    snapshot.pnlIntegrityCheck = { ok: _p6Integrity.ok, status: _p6Integrity.status, errors: _p6Integrity.errors, warnings: _p6Integrity.warnings, violations: _p6Integrity.errors }
    ...
    if (_walletIsContractLikeForPnl) {
      ...
      if (snapshot.walletTradeStatsSummary) {
        ...
        ts.publicWinRatePercent = null
        ts.winRateStatus = 'locked_small_sample'
        ts.publicPnlStatus = 'open_check'
        ts.publicPnlDisplayLabel = _contractLikeLabel
        ts.publicPnlDisplayReason = _contractLikeReason
        // WALLET-NO-PNL-UX-1: a non-trader address must report a specific, honest reason for
        // having no trader PnL instead of the generic 'no_closed_lots' bucket.
        ts.economicSignificance = 'open_check'
        ts.economicSignificanceReason = 'non_trader_address_type'
        ts.missing = [...(ts.missing ?? []).filter((m: string) => m !== 'no_closed_lots'), 'non_trader_address_type']
      }
      ;(snapshot as any).walletNoPnlReason = 'non_trader_address_type'
      ;(snapshot as any).walletNoPnlReasonLabel = 'Not applicable — token/distributor address'
      ;(snapshot as any).walletNoPnlNextAction = 'No action — this address type is not evaluated for trader PnL.'
      ;(snapshot as any).walletNoPnlCanRecover = false
    } else {
      // WALLET-NO-PNL-UX-2: for a normal_wallet (or unknown) address with real activity but no
      // closed FIFO lots, classify precisely why instead of leaving the UI to infer a generic
      // "no verified trades yet" from an empty closedLots count.
      const _rawClosedLotsForNoPnl = promotedTradeStatsSummary.rawClosedLots ?? promotedTradeStatsSummary.closedLots ?? 0
      const _activityEventsForNoPnl = walletFacts.activity?.eventCount ?? 0
      if (_activityEventsForNoPnl > 0 && _rawClosedLotsForNoPnl === 0) {
        const _budgetCappedBeforeRecovery = Boolean((snapshot._diagnostics as any)?.walletBudgetDebug?.budgetCapped)
        const _swapCandidateCount = walletFacts.sourceClassification?.swapLikeTxs ?? 0
        const _claimOrTransferOnly = (walletFacts.sourceClassification?.transferOnlyTxs ?? 0) + (walletFacts.sourceClassification?.claimOrAirdropLikeTxs ?? 0) >= _activityEventsForNoPnl - (walletFacts.sourceClassification?.unknownTxs ?? 0) && _swapCandidateCount === 0
        const _missingCounterpartyData = (walletFacts.activity?.unknownCount ?? 0) > 0 && (walletFacts.activity?.inboundCount ?? 0) + (walletFacts.activity?.outboundCount ?? 0) === 0
        const _historicalRecoveryNeeded = Boolean(_walletHistoricalScanDebug.eligible) && !_walletHistoricalScanDebug.notRunDueToCostGuard
        let _noPnlReason: string
        ...
        if (_budgetCappedBeforeRecovery) {
          ...
          _noPnlReason = 'no_wallet_initiated_transactions'; _noPnlLabel = 'No wallet-initiated transactions found'
          _noPnlNextAction = 'No action — this wallet has not initiated any on-chain transactions in the sample.'; _canRecover = false
        } else if (_missingCounterpartyData) {
          _noPnlReason = 'missing_counterparty_direction_data'; _noPnlLabel = 'Counterparty/direction data unavailable'
          _noPnlNextAction = 'Re-scan the exact chain to recover counterparty/direction data.'; _canRecover = true
        } else if (_claimOrTransferOnly) {
          _noPnlReason = 'transfer_or_airdrop_only_activity'; _noPnlLabel = 'Activity is transfer/airdrop-only — no swaps to price'
          _noPnlNextAction = 'No action — no swap-like activity exists to produce trader PnL.'; _canRecover = false
        } else if (_swapCandidateCount === 0) {
          _noPnlReason = 'no_swap_candidates'
          // WALLET-NO-PNL-UX-2: report the exact outcome of the low-cost receipt recovery attempt
          // (buildWalletPnlRecoveryV2Base) instead of a generic "scan full history" suggestion —
          // that recovery already ran against this wallet's own ranked target tokens above.
          if (walletPnlRecoveryV2Debug.attempted && walletPnlRecoveryV2Debug.priorBuysRecovered === 0) {
            _noPnlLabel = 'No swap candidates — low-cost recovery found no prior buy/sell legs'
          } else {
            _noPnlLabel = 'No swap candidates detected in sample'
          }
          _noPnlNextAction = 'Scan full history for swap evidence outside the current sample.'; _canRecover = true
        } else if (_historicalRecoveryNeeded) {
          _noPnlReason = 'historical_recovery_needed'; _noPnlLabel = 'Earlier buy-side history is needed to complete cost basis'
          _noPnlNextAction = 'Run a deeper historical scan to recover the missing buy leg(s).'; _canRecover = true
        } else {
          _noPnlReason = 'unsupported_router_or_unparsed_receipts'; _noPnlLabel = 'Swap activity found but could not be parsed'
          _noPnlNextAction = 'No action — this wallet uses a router/receipt shape not yet supported for recovery.'; _canRecover = false
        }
        ;(snapshot as any).walletNoPnlReason = _noPnlReason
    ...
    snapshot.walletProfileHints = deriveWalletProfileHints({
      ...
      winRatePercent: promotedTradeStatsSummary.scoreUnlocked === true ? (promotedTradeStatsSummary.winRatePercent ?? null) : null,
  ...
  snapshot.walletProfileDebug = {
    scoreInputs: {
      ...
      winRatePercent: snapshot.walletTradeStatsSummary?.scoreUnlocked === true ? (snapshot.walletTradeStatsSummary?.winRatePercent ?? null) : null,
      winRateStatus: snapshot.walletTradeStatsSummary?.scoreUnlocked === true && snapshot.walletTradeStatsSummary?.publicWinRatePercent != null ? 'unlocked' : 'locked_small_sample',
