...
const V3_SUBGRAPH_ID_BY_CHAIN: Partial<Record<'eth' | 'base' | 'bnb', string | undefined>> = {
...
// Uniswap V3 (and PancakeSwap V3, same subgraph schema) pools. Returns null (meaning "no real
// source available", never a fabricated empty-but-confident result) whenever the chain has no
// subgraph ID configured, the pool model isn't V3-family, or no pool contract address is known —
// in every such case the existing RPC-based candidate-probe/liquidity-only path runs unchanged.
export const resolveUniswapV3PositionOwners: ConcentratedOwnerResolver = async (input) => {
  if (input.poolModel !== 'uniswap_v3' && input.poolModel !== 'pancakeswap_v3') return null
  if (!input.poolAddress) return null
  const chain = input.chain as 'eth' | 'base' | 'bnb'
  const subgraphId = V3_SUBGRAPH_ID_BY_CHAIN[chain]
