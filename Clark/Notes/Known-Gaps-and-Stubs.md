...
3. **Unsupported Token Scanner chains.** `toTokenApiChain()` resolves Base, Ethereum, BNB, and Robinhood. It returns `null` for Polygon, Solana, Arbitrum, and unknown slugs — those token scans return "chain not yet supported," not a degraded/best-effort Base scan. See [[Supported-Chains-Limitations]].
