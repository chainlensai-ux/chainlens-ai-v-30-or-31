...
assert.match(snap, /function normalizeReceiptProofChain\(chain: string \| null \| undefined\): 'eth' \| 'base' \| null/, 'receipt proof normalizes candidate chains before routing')
assert.match(snap, /candidateChainRaw = candidateChainByTxHash\.get\(txHash\.toLowerCase\(\)\)/, 'receipt proof derives chain from the candidate tx/event itself')
assert.match(snap, /const receiptRpcUrl = alchemyReceiptRpcUrlForChain\(normalizedReceiptChain, alchemyUrl\)/, 'receipt proof selects the chain-specific Alchemy RPC URL')
assert.match(snap, /receiptProofCandidatesByChain/, 'receipt proof exposes aggregate candidates by chain')
assert.match(snap, /receiptProofCallsByChain/, 'receipt proof exposes aggregate RPC calls by chain')
assert.match(snap, /receiptProofNullByChain/, 'receipt proof exposes null receipts by chain')
assert.match(snap, /receiptProofChainMismatchCount/, 'receipt proof exposes candidate/RPC chain mismatch count')
assert.match(snap, /sampleReceiptProofChainMismatches/, 'receipt proof exposes sample candidate/RPC chain mismatches')
...
assert.match(snap, /candidateDebug\.rejectReason = decode\?\.shapeReason \?\? 'receipt_shape_unexpected'[\s\S]*?bumpRejected\(decode\?\.shapeReason \?\? 'receipt_shape_unexpected'\)/, 'receipt reconstruction rejects with the exact normalized receipt shape reason')
