...
// ─── Solana CORTEX Dev Control Read (mirrors EVM's Dev Control hero/tab structure) ─────────────
{
  ...
  check('Cluster Map renders an interactive node graph via SolanaClusterGraphPanel, not a text-only summary', devBlockRaw.includes('<SolanaClusterGraphPanel'))
  check('Cluster Map discloses its real scope limit (no cross-mint shared-signer/metadata-authority/treasury claims)', devBlockRaw.includes('this codebase does not perform'))
