...
const cacheSrc = fs.readFileSync(new URL('../lib/server/whaleAlertCache.ts', import.meta.url), 'utf8')
...
assert.match(feedSrc, /whaleFeedCache/, 'feed route must use the shared whale feed cache')
assert.match(cacheSrc, /export function clearWhaleFeedCache\(\)/, 'shared cache module must export clearWhaleFeedCache')
assert.match(syncSrc, /import \{ clearWhaleFeedCache \} from '@\/lib\/server\/whaleAlertCache'/, 'sync must import the shared cache invalidator')
