...

{
  ...
  assert.ok(/market cap/i.test(out), 'liquidity answers must include market cap when the format requires it')
  assert.ok(!/honeypot/i.test(out))
  assert.ok(out.includes('Pool address:'))
  assert.ok(out.includes('LP lock/burn:') || out.includes('LP/control'))
  assert.ok(out.includes('Controller:'))
  assert.ok(out.includes('Missing evidence:'))
  assert.ok(out.includes('Meaning:'))
  assert.ok(out.includes('Verdict:'))
  assert.ok(out.includes('Liquidity partial') || out.includes('Liquidity verified') || out.includes('Liquidity risky') || out.includes('Liquidity unavailable') || out.includes('unsupported') || /Verdict: (Strong|Decent|Thin|Risky|Partial)/i.test(out))
}

{
  ...
  assert.ok(routeSrc.includes('isForcedLiquidityCheckPrompt(prompt)') && routeSrc.includes('isTokenFollowupPrompt(prompt)'), 'token follow-up TOKEN READ path must not hijack LP prompts')
