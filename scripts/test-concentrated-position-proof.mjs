...
async function main() {
  ...
  // ── Public API + UI: no raw field names, no V4/V2 contradictions, exact status labels ──────
  {
    ...
    assert.ok(route.includes('this token’s primary pool is concentrated liquidity'))
    ...
    assert.ok(ui.includes('Unsupported — provider path unavailable'), 'UI control-proof wording includes the exact unsupported provider state')
    ...
    assert.ok(ui.includes('Position proof attempted — partial'), 'UI uses the exact partial status label')
    ...
    assert.ok(ui.includes('Unavailable — owner evidence missing'), 'UI uses a reason-bearing unavailable state for unresolved owners')
