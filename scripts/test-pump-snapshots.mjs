// PUMP INTERNAL SNAPSHOT TESTS, DISCLOSED (14d fallback fix): ChainLens-owned price snapshots must
// yield a real measured 14d change once enough history exists — and refuse to invent one when the
// window is too short or too stale.

import assert from 'node:assert/strict'
import { computeSnapshotChange14d, _resetSnapshotMemoryForTest, _seedSnapshotMemoryForTest } from '../lib/server/pump14dEvidence.ts'
...
// Not enough history → null change, honest refusal
{
  ...
  const r = await computeSnapshotChange14d('base', '0xAAA', now)
  assert.equal(r.changePct, null, 'a single snapshot can never produce a change figure')
}

// Enough history (~12 days apart) → real measured change
{
  _resetSnapshotMemoryForTest()
  _seedSnapshotMemoryForTest([
    {
      ...
      fdv_usd: 400_000, market_cap_usd: null, captured_at: new Date(now - 12.5 * DAY).toISOString(),
    },
    {
      chain: 'base', contract: '0xbbb', pair_address: '0xpair',
  ...
  const r = await computeSnapshotChange14d('base', '0xBBB', now)
  assert.ok(r.changePct != null, 'a ≥12-day snapshot span must produce a real measured change')
  assert.equal(Math.round(r.changePct), 50, 'change must be computed from real prices (0.001 → 0.0015 = +50%)')
}

// Too-short window (<12 days) → refuses to pass it off as a 14d figure
{
  _resetSnapshotMemoryForTest()
  _seedSnapshotMemoryForTest([
    {
      ...
      fdv_usd: 300_000, market_cap_usd: null, captured_at: new Date(now - 6 * DAY).toISOString(),
    },
    {
      chain: 'eth', contract: '0xccc', pair_address: null,
  ...
  const r = await computeSnapshotChange14d('eth', '0xCCC', now)
  assert.equal(r.changePct, null, 'a <12-day window is too short to call "14d" — refused')
}
