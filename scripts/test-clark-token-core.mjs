...
assert.ok(route.includes('tokenInternalApiPayload = { contract: tokenAddress, chain: toTokenApiChain(chain)'), '/api/token payload must use resolved ETH chain (no silent Base fallback)')
assert.ok(route.includes('const honeypotChain = toTokenApiChain(chain);'), 'independent security check must resolve chain explicitly (no silent Base default)')
