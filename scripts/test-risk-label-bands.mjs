...
check('58 copy is elevated-risk wording when holders are not verified', caution.copy === CAUTION_ELEVATED_COPY)
check('58 copy helper matches UI copy without holder evidence', riskLabelCopy(caution.riskLabel) === 'Elevated risk — missing LP/dev verification')
check('58 copy with verified holders is evidence-specific', riskLabelCopy(caution.riskLabel, { holdersVerified: true }) === 'Holder concentration and market data verified. LP proof incomplete; dev origin unresolved.')
check('58 copy with verified holders does not say missing holder evidence', !/missing holder/i.test(riskLabelCopy(caution.riskLabel, { holdersVerified: true }) ?? ''))
