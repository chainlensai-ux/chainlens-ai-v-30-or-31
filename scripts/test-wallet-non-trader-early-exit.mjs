...
assert.match(snap, /const _providerProfitEligible = _providerProfitDeepActivityRequested && _providerProfitFifoFoundNoLots && \(totalValue >= 1000 \|\| debug \|\| deepScan\) && _providerProfitBudgetOk/, 'provider PnL summary fallback requires deep activity, zero FIFO lots, a value/debug/deepScan trigger, and budget headroom')
...
assert.match(snap, /const _providerSummaryNoPnlReason = _possibleRelayedTrader \? 'relayed_trader_provider_summary_available' : 'provider_summary_available_fifo_missing'/, 'provider-trade-proven wallets get provider_summary_available_fifo_missing (or the relayed-trader variant) instead of a non-trader/no-PnL verdict')
assert.match(snap, /walletNoPnlReason = _providerSummaryNoPnlReason/, 'walletNoPnlReason is set from the provider-summary reason variable')
