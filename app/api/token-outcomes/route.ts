...
import { outcomeDb, refreshOutcomes } from '@/lib/server/tokenOutcomeService'
...
export async function GET(req: Request) {
  ...
  try {
    ...
    const { data, error } = await outcomeDb().from('tracked_token_outcomes').select('id,chain,token_address,scan_id,tracked_at,baseline_price_usd,baseline_liquidity_usd,baseline_market_cap_usd,baseline_risk_score,baseline_verdict,current_price_usd,current_liquidity_usd,price_change_pct,liquidity_change_pct,outcome_status,outcome_confidence,outcome_reasons_json,last_checked_at,market_source,baseline_snapshot_json->tokenSymbol,baseline_snapshot_json->tokenName,baseline_snapshot_json->scannedAt').eq('user_id', user.userId).order('tracked_at', { ascending: false }).limit(OUTCOME_POLICY.limits.elite)
    if (error) return json({ error: 'Outcome storage unavailable. The tracked-token-outcomes migration must be applied.' }, 503)
    const outcomes = (data ?? []).map(row => ({ ...row, baseline_snapshot_json: { tokenSymbol: row.tokenSymbol, tokenName: row.tokenName, scannedAt: row.scannedAt, baselineRiskScore: row.baseline_risk_score, baselineVerdict: row.baseline_verdict } }))
