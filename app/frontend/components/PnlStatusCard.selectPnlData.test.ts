  ...
  const base: PnlReconciliationSummary = {
    ...
    publicPnlGateAudit: {
      ...
      engineDivergenceDiagnostic: {
        fifoFragmentCount: 27,
        uniqueClosingTxCount: 9,
        uniqueClosingTxTokenCount: 9,
        pnlSellEntryCount: 9,
        avgFragmentsPerSell: 3,
        diagnosticOnly: true,
      },
...
describe('verified sample vs full-history UI (buildWalletPnlViewModel)', () => {
  ...
  function sampleSummary(overrides: Partial<PnlReconciliationSummary> = {}) {
    return reconciliationSummary({
      ...
      verifiedSamplePerformanceAudit: {
        ...
        roiEligibleLotCount: 98,
        quoteCashLegExcludedLotCount: 0,
        unresolvedQuoteLegLotCount: 0,
        roiUnavailableReason: null,
        realizedRoiPnlUsd: SAMPLE_PNL,
        sampleCostBasisUsd: SAMPLE_COST,
      },
      ...overrides,
    })
  }
  ...
  it('HARD ASSERTION: 98 verified sample + 2 unmatched sells → sample PnL/ROI display; Combined/full-wallet stays unavailable', () => {
    ...
    assert.match(vm.verifiedSampleRealizedBox.reason, /98 verified lots/)
    assert.match(vm.verifiedSampleRealizedBox.reason, /100% priced/)
    assert.equal(vm.roiBox.status, 'Partial')
    assert.equal(vm.roiBox.statusLabel, 'PARTIAL / VERIFIED BOUNDED SAMPLE')
    ...
    const vm = buildWalletPnlViewModel({
      ...
      reconciliationSummary: reconciliationSummary({
        ...
        verifiedSamplePerformance: {
          ...
          isCompleteWalletHistory: false,
          verifiedSampleRoiEligibleLots: [],
          verifiedSampleRoiEligibleLotCount: 1,
          quoteCashLegExcludedLotCount: 0,
          unresolvedQuoteLegLotCount: 0,
          roiUnavailableReason: null,
        },
