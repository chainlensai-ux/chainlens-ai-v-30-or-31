...
function getRadarScore(token: RadarToken): number {
...
function getStatus(token: RadarToken, score: number, momentum: MomentumLevel): RadarStatus {
  ...
  if (score >= 80 && momentum === 'HIGH') return 'HOT'
...
function enrichToken(token: RadarToken): TokenIntel {
  ...
  const radarScore = getRadarScore(token)
...
function TokenCard({ token, index, onClick }: { token: TokenIntel; index: number; onClick: () => void }) {
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', gap: '20px', marginBottom: '10px', flexWrap: 'wrap' }}>
        <Metric label='Radar Score' value={String(token.radarScore)} />
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
      ...
      >
        ...
        {['GeckoTerminal (pools)', 'Honeypot.is (security)'].map(src => (
          <div
            key={src}
            style={{
              display: 'flex',
...
export default function BaseRadarPage() {
  ...
  const filteredTokens = useMemo(() => {
    if (activeFilter === 'ALL') return intelTokens
    return intelTokens.filter(token => token.status === activeFilter)
  }, [activeFilter, intelTokens])
      ...
      >
        <div style={{ marginBottom: '32px' }}>
          ...
          <p style={{ fontSize: '13px', color: '#64748b', margin: '0 0 16px' }}>
            Base intelligence radar for new pools — score, momentum, and risk/opportunity signals.
          </p>
        ...
        >
          <div>
            ...
            >
              Live Feed {filteredTokens.length > 0 && `— ${filteredTokens.length} token${filteredTokens.length !== 1 ? 's' : ''}`}
            </p>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              {filteredTokens.map((token, i) => (
                <TokenCard key={`${token.contract}-${refreshKey}`} token={token} index={i} onClick={() => openToken(token.contract)} />
              ))}
            </div>
          </div>
