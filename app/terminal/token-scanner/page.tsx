  ...
  return (
    <div style={{ display:'grid', gap:'12px' }}>
      ...
      <div style={{ display:'grid', gridTemplateColumns:selectedClusterNodeId ? 'repeat(auto-fit,minmax(min(100%,280px),1fr))' : 'repeat(auto-fit,minmax(min(100%,280px),1fr))', gap:'12px', alignItems:'start' }}>
        <div onClick={() => { setSelectedClusterNodeId(null); clearEdgeHover() }} style={{ position:'relative', minHeight:'390px', borderRadius:'16px', overflow:'hidden', background:`radial-gradient(circle at 50% 48%, ${riskTint}, transparent 42%), linear-gradient(145deg, rgba(3,10,24,.98), rgba(8,16,32,.95))`, border:'1px solid rgba(125,211,252,.16)' }}>
          <svg viewBox="0 0 100 100" preserveAspectRatio="none" style={{ position:'absolute', inset:0, width:'100%', height:'100%', zIndex:1 }}>
            {graphEdges.map((edge) => {
              ...
              const isHovered = hoveredClusterEdgeId === edge.id
              ...
              const strokeOpacity = selectedClusterNodeId ? (isConnected ? 1 : 0.12) : edge.opacity
              const strokeWidth = clamp(edge.width + (isConnected ? 1 : 0), 1, 4.5)
              return (
                <g key={edge.id}>
                  <path d={path} fill="none" stroke={isConnected || isHovered ? '#e0f2fe' : edge.color} strokeWidth={strokeWidth} strokeOpacity={strokeOpacity} strokeLinecap="round" style={{ filter: isConnected || isHovered ? `drop-shadow(0 0 7px ${edge.color})` : undefined }} />
          ...
          <div style={{ position:'absolute', left:'12px', bottom:'12px', display:'flex', flexWrap:'wrap', gap:'6px', zIndex:3 }}>
            {[
              ['#fbbf24','Deployer'], ['#2dd4bf','Linked wallet'], ['#e879f9','Cluster wallet'], ['#64748b','Holder wallet'], ['#38bdf8','Transfer edge'], ['#a855f7','Cluster edge'], ['#facc15','Shared pattern'], ['#fb7185','Suspicious edge'],
